package com.cedricmartens.hexpert.social;

/**
 * Created by martens on 5/24/17.
 */

public class Achievement
{
    public static final String PERFECT = "CgkIq5XDzvcOEAIQAA";
    public static final String TO_SPACE = "CgkIq5XDzvcOEAIQAQ";
    public static final String TOO_BIG_TO_FAIL = "CgkIq5XDzvcOEAIQAg";
    public static final String SPACE_RACE = "CgkIq5XDzvcOEAIQAw";
    public static final String GREAT_ESCAPE = "CgkIq5XDzvcOEAIQCQ";
    public static final String EFFICIENT_ROCKET = "CgkIq5XDzvcOEAIQCw";
    public static final String DOUBLE_THE_NEGATIVE = "CgkIq5XDzvcOEAIQDQ";
    public static final String HARDCORE_PLAYER = "CgkIq5XDzvcOEAIQDA";
    public static final String USELESS_PROPERTY = "CgkIq5XDzvcOEAIQCg";

    public static final String NOVICE = "CgkIq5XDzvcOEAIQBA";
    public static final String AMATEUR = "CgkIq5XDzvcOEAIQBQ";
    public static final String PROFESSIONAL = "CgkIq5XDzvcOEAIQBg";
    public static final String HEXPERT = "giCgkIq5XDzvcOEAIQBw";
}
